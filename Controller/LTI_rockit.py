...
class MPC:
    ...
    def __init__(self, vehicle, Q, R, P0, process_noise, Possibility=0.95, N=12) -> None:
        ...
        self.x = self.ocp.state(self.nx,1)
        self.u = self.ocp.control(self.nu,1)
        ...
        self.lambda_s= self.ocp.variable(1,self.N + 1)
        # slack variable for y 
        self.slack_y = self.ocp.variable(1,self.N + 1)
        # ! here is a test for sigma
        self.sigma = self.ocp.variable(1,self.N + 1)

        # IDM leading vehicle position parameter
        self.p_leading = self.ocp.parameter(2,1)
        # set car velocity 
        self.leading_velocity = self.ocp.parameter(1)
        self.vel_diff = self.ocp.parameter(1)
    ...
    def setStateEqconstraints(self, v=15, phi=0, delta=0):
        ...
        self.A, self.B, self.C = A, B, C
            # self.ocp.subject_to(self.x[:, i+1] == A @ self.x[:, i] + B @ self.u[:, i] + C)
            # self.ocp.set_der(self.x, self.vehicle.model_xdot())'
        self.x_next = A @ self.x + B @ self.u + C
        self.ocp.set_der(self.x, self.x_next )
        self.ocp.subject_to(self.ocp.at_t0(self.x) == self.x0)
    ...
    def setInEqConstraints(self):
        ...
        for i in range(self.N+1):
            ...
            # ! Set the IDM constraint
            self.ocp.subject_to(self.x[0, i] - self.tightened_bound_N_IDM_list[i].item() <= 0)
            
            # ! Set the vel_diff constraint
            # self.ocp.subject_to(self.x[2, i] - self.leading_velocity <= self.tightened_bound_N_vel_diff_list[i].item())
            # self.ocp.subject_to(self.x[2, i] - self.leading_velocity >= -self.tightened_bound_N_vel_diff_list[i].item())
            # ! set the y constraint
            # self.ocp.subject_to(self.x[1, i] <= self.tightened_bound_N_y_upper_list[i].item())
            # self.ocp.subject_to(self.x[1, i] >= self.tightened_bound_N_y_lower_list[i].item())
            
        # set the constraints for the input  [-3.14/8,-0.7*9.81],[3.14/8,0.05*9.81]
        self.ocp.subject_to(self.u[0, :] >= -3.14 / 8)
        self.ocp.subject_to(self.u[0, :] <= 3.14 / 8)
        self.ocp.subject_to(self.u[1, :] >= -0.5 * 9.81)
        self.ocp.subject_to(self.u[1, :] <= 0.5 * 9.81)
        self.ocp.subject_to(self.lambda_s <= 0)
        # self.ocp.subject_to(self.slack_y >= 0)
        # ! here is test for sigma
        for i in range(self.N+1):
            self.ocp.subject_to(self.sigma[0,i] >= -3e4*self.lambda_s[0,i])
            self.ocp.subject_to(self.sigma[0,i] >= 3e4*self.lambda_s[0,i]**2)
    ...
    def setCost(self):
        ...
        self.ocp.add_objective(self.ocp.at_tf(self.cost))
    ...
    def solve(self, x0, ref_trajectory, ref_control, p_leading, leading_velocity=10, vel_diff=6):
        ...
        self.ocp.set_value(self.refx, ref_trajectory)
        self.ocp.set_value(self.refu, ref_control)
        self.ocp.set_value(self.p_leading, p_leading)
        self.ocp.set_value(self.leading_velocity, leading_velocity)
        self.ocp.set_value(self.vel_diff, vel_diff)
