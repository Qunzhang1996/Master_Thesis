...
class combinedTraffic:
    ...
    def __init__(self,vehicles,egoVehicle,N,f_controller, respawnTol = 75, respawnPos = 120):
        ...
        self.N = vehicles[0].getDim()[1]
        self.nx = np.max([vehicles[i].getDim()[0] for i in range(self.Nveh)])
    ...
    def prediction(self):
        ...
        for i in range(self.Nveh):
            prediction_i = self.vehicles[i].prediction(self.N_pred)
            if prediction_i.shape[0] == self.nx:
                self.states[:,:,i] = prediction_i 
            else: 
                self.states[:,:,i] = np.append(prediction_i,np.zeros((1,self.N_pred+1)),axis = 0)
    ...
    def getStates(self):
        ...
        for i in range(self.Nveh):
            state_i = np.squeeze(self.vehicles[i].getState())
            states[:,i] = state_i if len(state_i) == self.nx else np.append(state_i,0)
        return states

    def getFeatures(self):
        self.n_features = self.nx if self.nx == 5 else self.nx+1
        features = np.zeros((self.n_features,self.Nveh))
        for i in range(self.Nveh):
            features_i = np.squeeze(self.vehicles[i].getFeatures())
            features[:,i] = features_i[:3] if len(features)>5 else features_i
        return features
...
class vehicleSUMO:
    """
    A relistic traffic simulator for a passenger vehicle
    ...
    def setScenario(self,scenario):
        _,_, self.laneCenters,_ = scenario.getRoad()
    ...
    def getFeatures(self):
        # Returns current state of the vehicle
        return np.array([*self.p,self.v,self.theta,self.typeEncoding])
...
class vehicleSemiSUMO:
    """
    A relistic traffic simulator of a semi-truck vehicle
    IDM for acceleration, MOBIL for lane change and PI controller for steering angle
    """
    def __init__(self,dt,N,p0,v0,width = 2.54, length = 16.1544, type = "normal"):
        """
        type - setting for vehicle behavoir 
            normal : Close to speed limit, considers other vehicles
            passive : Under speed limit, high consideration of other vehicles
        ...
        self.theta_1  = 0.0
        self.theta_2  = 0.0
        self.state = np.zeros((5,self.N+1),dtype=float)             # [p_x, p_y, v_x, theta, steering angle, acceleration]
        self.state[:2,0] = self.p
        self.state[2,0] = self.v0_x

        self.u = np.zeros((2,N), dtype= float)

        # !! TODO Adapt parameters to fit for a semi-trailer !!

        # Set model parameters
        if type == "passive_truck":
            self.typeEncoding = 1
            self.IDM_s0 = 5
            self.IDM_T  = 2.5
            self.IDM_amax = 0.5
            ...
            # Bonus distance + velocity difference considered for truck
            self.truck_respect_s = 5
            self.truck_respect_v = 2

        elif type == "aggressive_truck":
            self.typeEncoding = 3
            self.IDM_s0 = 2
            self.IDM_T  = 1.5
            self.IDM_amax = 1
    ...
    def model(self,x_init):
        ...
        dp_y = x_init[2] * np.tan(x_init[3])
        dv_x = self.u[1,0]
        dtheta_1 = x_init[2] / self.L_tract * np.tan(self.u[0,0]) / np.cos(x_init[3])
        dtheta_2 = x_init[2] / self.L_tract* np.sin(x_init[3] - x_init[4]) / np.cos(x_init[3])
        dX = np.array([dp_x,dp_y,dv_x,dtheta_1,dtheta_2])
        return dX
    ...
    def move(self,x_init):
        ...
        return np.array([*p,self.v,self.theta_1,self.theta_2])

    def prediction(self,N_pred):
        # Makes prediction of traffic movement over horizon
        self.state = np.zeros((self.nx,N_pred+1))
        self.state[:,0] = self.getState() #np.array(np.append(np.append(self.p, self.v),self.theta_1))
        for i in range(1,N_pred+1):
            self.state[:,i] = self.move(self.state[:,i-1])
    ...
    def pushUpdate(self):
        p_next = self.state_next[:2]
        v_next = self.state_next[2]
        theta1_next  = self.state_next[3]
        theta2_next  = self.state_next[4]
        
        self.p = p_next.tolist()
        self.v = v_next
        self.theta_1 = theta1_next
        self.theta_2 = theta2_next

        self.setLane()

    def getState(self):
        # Returns current state of the vehicle
        return np.array([*self.p,self.v,self.theta_1,self.theta_2])

    def getFeatures(self):
        # Returns current state of the vehicle
        return np.array([*self.p,self.v,self.theta_1,self.typeEncoding])
