...
class makeController:   
    ...
    def __init__(self, vehicle,traffic,scenario,N,opts,dt,stochasticMPC=True):
    ...
    def setInEqConstraints_val(self, H_up=None, upb=None, H_low=None, lwb=None):
        ...
        self.H_up_u = [np.array([[1], [0]]), np.array([[0], [1]])]
        self.upb_u = np.array([[3.14/8], [0.5*9.81]])
        self.H_low_u = [np.array([[-1], [0]]), np.array([[0], [-1]])]
        self.lwb_u = np.array([[3.14/8], [0.5*9.81]])
    ...
    def setInEqConstraints(self):
        ...
        if self.stochasticMPC:
            ...
            self.tightened_bound_N_U_list_up = self.MPC_tighten_bound.tighten_bound_N_U(self.P0, self.H_up_u, self.upb_u, self.N, 1)
            self.tightened_bound_N_U_list_lw = self.MPC_tighten_bound.tighten_bound_N_U(self.P0, self.H_low_u, self.lwb_u, self.N, 0)
            ...
            for i in range(self.N):
                self.opti.subject_to(self.u[:, i] <= DM(self.tightened_bound_N_U_list_up[i].reshape(-1, 1)))
                self.opti.subject_to(self.u[:, i] >= DM(self.tightened_bound_N_U_list_lw[i].reshape(-1, 1)))
...
class makeDecisionMaster:
    ...
    def updateReference(self, r=np.zeros((4,1))):
        ...
        tol = 0.2
        if py_ego >= self.laneCenters[1]-0.5:
            ...
            print("executed  left lane change!!!!!!!!!!!")

        elif py_ego <  self.laneCenters[2]+0.5:
